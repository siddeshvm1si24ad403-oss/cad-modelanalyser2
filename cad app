...
def convert_step_to_stl(step_path, stl_path):
    try:
        ...
        cq.importers.importStep(step_path).val().exportStl(stl_path)
        if os.path.exists(stl_path) and os.path.getsize(stl_path) > 0:
            return True, "CadQuery", "cadquery"
    except: pass
    try:
        import sys
        for p in ['/Applications/FreeCAD.app/Contents/Resources/lib',
                  '/Applications/FreeCAD_1.0.app/Contents/Resources/lib',
                  '/usr/lib/freecad/lib']:
            if os.path.exists(p) and p not in sys.path: sys.path.append(p)
        import FreeCAD, Import, Mesh
        Import.insert(step_path, "T")
        doc = FreeCAD.ActiveDocument
        if not doc: doc = FreeCAD.newDocument("T"); Import.insert(step_path,"T")
        Mesh.export(list(doc.Objects), stl_path)
        try: FreeCAD.closeDocument("T")
        except: pass
        if os.path.exists(stl_path) and os.path.getsize(stl_path) > 0:
            return True, "FreeCAD", "freecad"
    except: pass
    try:
        import subprocess, tempfile as _tf
        script = f'import FreeCAD,Import,Mesh\nImport.insert("{step_path}","T")\nd=FreeCAD.ActiveDocument\nMesh.export(list(d.Objects),"{stl_path}")\nFreeCAD.closeDocument("T")\n'
        ...
        for cmd in ['freecadcmd','FreeCADCmd',
            ...
            try:
                ...
                if r.returncode==0 and os.path.exists(stl_path) and os.path.getsize(stl_path)>0:
                    os.remove(sf.name); return True, "FreeCAD CLI", "freecad_cli"
            except: continue
    ...
    try:
        ...
        from step_converter_pure import convert_step_to_stl as _p
        ok, msg = _p(step_path, stl_path)
        if ok and os.path.exists(stl_path) and os.path.getsize(stl_path) > 0:
            return True, "fallback (simplified)", "fallback"
    except: pass
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if ext == '.stl':
                ...
                if not ok:
                    ...
                    st.error("Conversion failed. Install CadQuery or upload as STL.")
                    st.stop()
